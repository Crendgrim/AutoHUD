...
import mod.crend.autohud.mixin.*;
...
public class CrosshairHandler {
    ...
    public static final Identifier crosshairTexture = new Identifier("autohud", "textures/gui/crosshairs.png");
    enum ModifierUse {
        NONE,
        USE_ITEM,
        INTERACTABLE;
    ...
    private static void checkBreakable(ClientPlayerEntity player, Item handItem, HitResult hitResult) {
        if (AutoHud.config.dynamicCrosshairHoldingTool() == CrosshairPolicy.Disabled) return;
        if (AutoHud.config.dynamicCrosshairStyle() && hitResult.getType() == HitResult.Type.BLOCK) {
    ...
    private static boolean checkTool(ClientPlayerEntity player, Item handItem, HitResult hitResult) {
        if (!policyMatches(AutoHud.config.dynamicCrosshairHoldingTool(), hitResult)) return false;
    ...
    private static boolean canUseItem(ClientPlayerEntity player, ItemStack handItemStack, HitResult hitResult, Item handItem) {
        if (AutoHud.config.dynamicCrosshairHoldingUsableItem() == BlockCrosshairPolicy.Disabled) return false;
        if (AutoHud.config.dynamicCrosshairHoldingUsableItem() == BlockCrosshairPolicy.Always
        ...
        if (hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (handItem instanceof BucketItem) {
                ...
                if (handItem == Items.WATER_BUCKET || handItem == Items.LAVA_BUCKET) {
                    if (block.equals(Blocks.CAULDRON) && !player.shouldCancelInteraction()) return true;
                    if (AutoHud.config.dynamicCrosshairHoldingBlock() != BlockCrosshairPolicy.Disabled) {
                        activeCrosshair = AutoHud.config.getCrosshairHoldingBlock();
                        return false;
                    }
                    return true;
        ...
        } else if (hitResult.getType() == HitResult.Type.MISS && AutoHud.config.dynamicCrosshairHoldingBlock() == BlockCrosshairPolicy.Always) {
            if (handItem == Items.WATER_BUCKET || handItem == Items.LAVA_BUCKET) {
                activeCrosshair = AutoHud.config.getCrosshairHoldingBlock();
                return false;
            }
    ...
    private static boolean isBlockInteractible(ClientPlayerEntity player, HitResult hitResult, ItemStack mainHandStack) {
        ...
        if (AutoHud.config.dynamicCrosshairOnBlock() != InteractableCrosshairPolicy.Disabled && hitResult.getType() == HitResult.Type.BLOCK && !cancelInteraction) {
            BlockState blockState = MinecraftClient.getInstance().world.getBlockState(((BlockHitResult) hitResult).getBlockPos());
            Block block = blockState.getBlock();
            if (block instanceof BlockWithEntity) {
    ...
    private static ModifierUse checkEntityModifiers(ClientPlayerEntity player, ItemStack itemStack, HitResult hitResult) {
        ...
        if (target.getType() == EntityType.ARMOR_STAND) return ModifierUse.USE_ITEM;
            ...
            return ModifierUse.NONE;
        } else if (target.getType() == EntityType.PARROT) {
            ...
            MerchantEntity merchant = ((MerchantEntity) target);
    ...
    private static boolean checkShowCrosshair() {
        ...
        activeCrosshair = AutoHud.config.getCrosshairRegular();
        ...
        if (AutoHud.config.dynamicCrosshairOnEntity() && hitResult.getType() == HitResult.Type.ENTITY) {
            activeCrosshair = AutoHud.config.getCrosshairOnEntity();
            useEntityCrosshair = true;
        }

        if (AutoHud.config.dynamicCrosshairOnBlock() == InteractableCrosshairPolicy.IfTargeting && hitResult.getType() == HitResult.Type.BLOCK) {
            activeCrosshair = AutoHud.config.getCrosshairOnBlock();
        }
    ...
    public static List<CrosshairModifier> getActiveCrosshairModifiers() {
        ...
        switch (modifierHit) {
            case CORRECT_TOOL -> modifiers.add(AutoHud.config.crosshairModCorrectTool());
            case INCORRECT_TOOL -> modifiers.add(AutoHud.config.crosshairModIncorrectTool());
        }
        switch (modifierUse) {
            case USE_ITEM -> modifiers.add(AutoHud.config.crosshairModUsableItem());
            case INTERACTABLE -> modifiers.add(AutoHud.config.crosshairModInteractable());
        }
        return modifiers;
    }
